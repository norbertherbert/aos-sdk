...
typedef enum {
	...
	aos_lpm_requester_rtc_alarm_a,		//!< The requester is the RTC alarm A
	aos_lpm_requester_rtc_alarm_b,		//!< The requester is the RTC alarm B
...
/*!
 * \struct aos_lpm_usage_duration_t
 *
 * \brief GNSS usage duration. To be used for power consumption estimation
 */
typedef struct {
	uint64_t running;				//!< Duration when MCU is running (step 100us)
	uint64_t sleep;					//!< Duration when MCU in in sleep mode (step 100 us)
	uint64_t stop1;					//!< Duration when MCU is in STOP1 (step 100 us)
	uint64_t stop2;					//!< Duration when MCU is in STOP2 (step 100 us)
} aos_lpm_usage_duration_t;
...
/*!
 * \fn void aos_lpm_init(void)
 *
 * \brief Initialize the LPM
 *
 * \note Intenally called by aos_system_init
 */
void aos_lpm_init(void);
...
/*!
 * \fn const aos_lpm_usage_duration_t* aos_lpm_get_usage(void)
 *
 * \brief Return the CPU usage
 *
 * \return The CPU usage
 */
const aos_lpm_usage_duration_t* aos_lpm_get_usage(void);

/*!
 * \fn void aos_lpm_clear_usage(void)
 *
 * \brief Clear the CPU usage.
 */
void aos_lpm_clear_usage(void);

/*!
 * \fn uint64_t aos_lpm_get_consumption_uah(void)
 *
 * \brief Retrieve the CPU energy consumption
 *
 * \return CPU energy consumption
 */
uint64_t aos_lpm_get_consumption_uah(void);

/*!
 * \fn void aos_lpm_clear_consumption(void)
 *
 * \brief Clear the CPU consumption
 */
void aos_lpm_clear_consumption(void);
