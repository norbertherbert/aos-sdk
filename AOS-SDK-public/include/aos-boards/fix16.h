...
// Abeeway. Tune the lib for our need.
...
// Max buffer size required to display a fix16
#define MAX_DISPLAY_FIX16_STR 14
...
static const fix16_t THREE_PI_DIV_2 = 0x4b65f;
static const fix16_t PI_DIV_2 = 0x19220;
static const fix16_t PI_MUL_2 = 0x6487f;
...
#define FIX16_TEN 		0x000a0000                 /*!< fix16_t value of 10 */
#define FIX16_THOUSAND 	0x03E80000                 /*!< fix16_t value of 1000 */
...
static inline fix16_t fix16_from_dbl(double a)
{
	...
	return (fix16_t)temp;
        ...
        // n must be strictly less than 16
static inline fix16_t fix16_from_Qn(int16_t a, int8_t n)
{
    if (n >= 16) {
	return a;
    }
...
extern fix16_t fix16_rad_to_degree(fix16_t rad)FIXMATH_FUNC_ATTRS;
...
/*! Return the shifted value of the given fix16_t (multiply or divide by two)
 *  if shift > 0: right shift (divide by 2)
 *  if shift < 0: left shift (multiply by 2)
 */
extern fix16_t fix16_shift(fix16_t x, int8_t shift);
 ...
 */
extern fix16_t fix16_pow(fix16_t x, fix16_t y);
 ...
 */
extern fix16_t fix16_from_str(const char *buffer);
